...
import { fetchLicensePlatesQuery } from "../app/hooks/useLicensePlates"
...
export default function LicensePlates({token}: LicensePlatesProps) {

    const { data } = useQuery<UserLicensePlates[]>({
        queryKey: ['license-plates'],
        queryFn: () => fetchLicensePlatesQuery(token),
        enabled: !!token
    });
return (
    <div className="bg-grey-5 p-s">
        {(data ?? []).map((licenseplate, index) => (
            <div key={index} className="grid gap-4xs">
            ...
            <div>
                <p>
                    {licenseplate.user_license_plate}
                </p>
            </div>
        </div>
            </div>
