...
const pauseMembershipFetch = async (pauseMonths: number) => {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL}/api-pause-membership`,
    {
      method: "PATCH",
      headers: {
...
const reactivateMembershipFetch = async () => {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL}/api-reactivate-membership`,
    {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${getToken()}`,
...
const updateMembershipFetch = async (membership: string) => {
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL}/api-update-membership`,
    {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
...
export function useMembership() {
  ...
  const updateMutation = useMutation({
    mutationFn: (membership: string) => updateMembershipFetch(membership),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["user"] }),
  });
